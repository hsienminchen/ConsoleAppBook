// 定義一個靜態方法，用來執行測試程式
using ConsoleAppBookSQLite;

//string connstr = @"Data Source=BingSQLite.db"; // 指定 SQLite 資料庫檔案的路徑
string connstr = @"Data Source=D:\OliverChen\dotnetLearning\ConsoleApp\ConsoleAppBook\ConsoleAppBookSQLite\BingSQLite.db"; // 指定 SQLite 資料庫檔案的路徑

// 建立一個 BookTable 物件，表示一個收藏書籍/電子書的資料表
BookTable bookTable = new BookTable(connstr);
